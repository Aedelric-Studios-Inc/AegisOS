...
int tcp_accept(int listener_id) {
    if (listener_id < 0 || listener_id >= TCP_MAX_CONNECTIONS) return -1;
    tcp_conn_t *listener = &connections[listener_id];
    if (!listener->active || listener->state != TCP_LISTEN) return -1;
    for (int i = 0; i < TCP_MAX_CONNECTIONS; i++) {
        tcp_conn_t *c = &connections[i];
        if (!c->active || c->state != TCP_ESTABLISHED || c->accepted) continue;
        if (c->local_port != listener->local_port || c->remote_ip == 0) continue;
        c->accepted = true;
        return i;
    }
    return -1;
...
int tcp_recv_id(int conn_id, u8 *buf, u16 max_len) {
    if (conn_id < 0 || conn_id >= TCP_MAX_CONNECTIONS || !buf || max_len == 0) return -1;
    tcp_conn_t *c = &connections[conn_id];
    if (!c->active || (c->state != TCP_ESTABLISHED && c->state != TCP_CLOSE_WAIT)) return -1;
    if (c->rx_len == 0) return 0;
    u32 n = c->rx_len < max_len ? c->rx_len : max_len;
    for (u32 i = 0; i < n; i++) buf[i] = c->rx_buf[i];
    for (u32 i = n; i < c->rx_len; i++) c->rx_buf[i - n] = c->rx_buf[i];
    c->rx_len -= n;
    return (int)n;
}

int tcp_send_id(int conn_id, const u8 *data, u16 len) {
    if (conn_id < 0 || conn_id >= TCP_MAX_CONNECTIONS || !data || len == 0) return -1;
    tcp_conn_t *c = &connections[conn_id];
    if (!c->active || c->state != TCP_ESTABLISHED) return -1;
    u16 off = 0;
    while (off < len) {
        u16 chunk = (u16)((len - off) > TCP_MSS ? TCP_MSS : (len - off));
        if (tcp_send_segment(c, TCP_FLAG_ACK | TCP_FLAG_PSH, data + off, chunk) != 0) return -1;
        off = (u16)(off + chunk);
    }
    return (int)len;
}

int tcp_close_id(int conn_id) {
    if (conn_id < 0 || conn_id >= TCP_MAX_CONNECTIONS) return -1;
    tcp_conn_t *c = &connections[conn_id];
    if (!c->active) return 0;
    if (c->state == TCP_LISTEN || c->state == TCP_CLOSED || c->state == TCP_TIME_WAIT) {
        c->active = false;
        c->state = TCP_CLOSED;
        return 0;
    }
    if (c->state == TCP_ESTABLISHED) {
        ...
        return tcp_send_segment(c, TCP_FLAG_FIN | TCP_FLAG_ACK, NULL, 0);
    }
...
bool tcp_is_established(int conn_id) {
    return conn_id >= 0 && conn_id < TCP_MAX_CONNECTIONS && connections[conn_id].active && connections[conn_id].state == TCP_ESTABLISHED;
}
