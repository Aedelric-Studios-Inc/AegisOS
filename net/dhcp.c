...
#define DHCP_MAGIC_COOKIE  0x63538263U /* bytes 63 82 53 63 on little-endian ARM */
...
static u32 dhcp_load_le32_unaligned(const u8 *p) {
    return ((u32)p[0]) |
           ((u32)p[1] << 8) |
           ((u32)p[2] << 16) |
           ((u32)p[3] << 24);
...
static u32 dhcp_load_be32_unaligned(const u8 *p) {
    return ((u32)p[0] << 24) |
           ((u32)p[1] << 16) |
           ((u32)p[2] << 8) |
           ((u32)p[3]);
...
void dhcp_handle_response(const u8 *data, u16 len) {
    ...
    while (i < 312 && opts[i] != 255) {
        ...
        case 1:  /* Subnet mask */
            if (opt_len >= 4) {
                dhcp_ctx.subnet_mask = dhcp_load_le32_unaligned(&opts[i]);
            }
            break;
        case 3:  /* Router */
            if (opt_len >= 4) {
                dhcp_ctx.gateway = dhcp_load_le32_unaligned(&opts[i]);
            }
            break;
        case 6:  /* DNS */
            if (opt_len >= 4) {
                dhcp_ctx.dns_server = dhcp_load_le32_unaligned(&opts[i]);
            }
            break;
        case 51: /* Lease time */
            if (opt_len >= 4) {
                dhcp_ctx.lease_time = dhcp_load_be32_unaligned(&opts[i]);
            }
            break;
        case 54: /* Server identifier */
            if (opt_len >= 4) {
                dhcp_ctx.server_ip = dhcp_load_le32_unaligned(&opts[i]);
            }
            break;
        }
        i += opt_len;
...
u32 dhcp_get_subnet_mask(void) { return dhcp_ctx.subnet_mask; }
u32 dhcp_get_gateway(void) { return dhcp_ctx.gateway; }
u32 dhcp_get_dns_server(void) { return dhcp_ctx.dns_server; }
u32 dhcp_get_lease_seconds(void) { return dhcp_ctx.lease_time; }
