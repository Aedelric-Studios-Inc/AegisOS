...
static u16 ip_checksum(const void *data, u16 len) {
    ...
    /* IPv4 headers received from Ethernet are not guaranteed to be aligned.
     * Assemble each 16-bit word from bytes so ARM64 never performs an
     * alignment-sensitive halfword load from the packet buffer.  The byte
     * order intentionally matches the previous little-endian u16 loads, so
     * checksum comparison/storage behaviour remains unchanged.
     */
    while (len > 1) {
        sum += (u16)bytes[0] | ((u16)bytes[1] << 8);
        bytes += 2;
        len -= 2;
    }
    if (len == 1) {
...
int ipv4_send(u32 dst, u8 proto, const u8 *payload, u16 len) {
    ...
    /* Limited broadcast is sent directly to the Ethernet broadcast address.
     * This is required before the interface has an address/default route,
     * particularly for DHCP DISCOVER and REQUEST.
     */
    if (dst == 0xFFFFFFFFU) {
        static const u8 broadcast[6] = {0xff,0xff,0xff,0xff,0xff,0xff};
        return ethernet_tx(broadcast, 0x0800, pkt);
    }
