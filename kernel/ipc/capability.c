...
bool cap_is_valid(cap_token_t tok) {
    u64 idx = tok;
    return idx != 0 && idx < MAX_CAPS && cap_table[idx].valid;
}

u32 cap_owner_tid(cap_token_t tok) {
    u64 idx = tok;
    if (idx == 0 || idx >= MAX_CAPS || !cap_table[idx].valid) return 0;
    return cap_table[idx].owner_tid;
}

u64 cap_rights(cap_token_t tok) {
    u64 idx = tok;
    if (idx == 0 || idx >= MAX_CAPS || !cap_table[idx].valid) return 0;
    return cap_table[idx].rights;
}

s64 cap_delegate(cap_token_t cap_tok, u32 target_tid, u64 rights) {
    ...
    cap_token_t new_tok = cap_create(src->owner_tid, target_tid, delegated_rights);
