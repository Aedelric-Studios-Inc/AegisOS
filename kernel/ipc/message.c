...
int ipc_send(u32 channel_id, const void *data, u64 len) {
    if (!data || len == 0 || len > MSG_MAX_PAYLOAD) return AEGIS_EINVAL;
...
int ipc_recv(u32 channel_id, void *buf, u64 max_len) {
     ...
     * v43.1: in QEMU smoke proofs this must be bounded so the IPC
     * roundtrip proof fails loudly instead of stalling the boot chain.
     */
    /* v55.1.8: early release/interactive runtime must never block the init
     * task before ttyAMA0 is online.  Return EBUSY for empty queues; userland
     * can retry once the scheduler/userspace runtime is mature.
     */
    if (q->count == 0) return AEGIS_EBUSY;
