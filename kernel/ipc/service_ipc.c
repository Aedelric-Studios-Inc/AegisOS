...
int service_ipc_message_roundtrip_selftest(void) {
    ...
    /* v55.1.4 P0: the milestone proof must exercise the actual low-level
     * channel queue path.  The payload remains kernel-owned/aligned so the
     * proof does not depend on unsafe EL0 user pointers, but delivery now goes
     * through ipc_send()/ipc_recv() instead of a local stack-buffer copy.
     */
    char recv[AEGIS_IPC_TEST_PAYLOAD_MAX];
    zero_bytes(recv, sizeof(recv));

    if (expected_len > (u64)sizeof(recv)) return AEGIS_EINVAL;

    int rc = ipc_send(mb->channel_id, (const void *)v43_4_ipc_test_payload, expected_len);
    if (rc != AEGIS_OK) return rc;
#ifdef AEGISOS_QEMU_SMOKE
    qemu_boot_checkpoint_ipc_roundtrip_send_done();
#endif

    rc = ipc_recv(mb->channel_id, recv, sizeof(recv));
    if (rc < 0) return rc;
    if ((u64)rc != expected_len) return AEGIS_EINVAL;
#ifdef AEGISOS_QEMU_SMOKE
