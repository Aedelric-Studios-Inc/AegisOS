...
#define TIMER_IRQ       30U  /* ARM PPI: physical timer */
#define KERNEL_TICK_US 10000ULL
...
/* Called from IRQ dispatch on each physical timer tick.
 *
 * The ARM generic timer is one-shot hardware.  Rearm it before returning to
 * the IRQ dispatcher.  Preemptive scheduling is intentionally not requested
 * here until AegisOS has an exception-frame-aware context-switch path.
 */
static void timer_irq_handler(u32 irq, void *data) {
    (void)irq;
    (void)data;

    timer_rearm_periodic();
    tick_count++;
    jiffies++;

    /* Check for expired software timers. */
    ...
    for (int i = 0; i < MAX_TIMERS; i++) {
        ...
        if (now >= timers[i].expire_ticks) {
            ...
            if (timers[i].periodic) {
                ...
                timers[i].expire_ticks = now +
                    (timers[i].interval_us * (freq / 1000000ULL));
...
void kernel_timer_init(void) {
    ...
    irq_register(TIMER_IRQ, timer_irq_handler, NULL);
    gic_enable_irq(TIMER_IRQ);
    timer_set_periodic(KERNEL_TICK_US, NULL);
}
