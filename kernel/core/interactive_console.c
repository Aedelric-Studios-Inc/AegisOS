...
/* The console task is created before PID 1 enters EL0 so it is available as
 * a scheduler successor, but it must not print a prompt until the native
 * service-manager has completed the aegisd IPC health roundtrip.  Otherwise
 * the background supervisor logs overwrite an already-active prompt and the
 * release VM appears to stall at the final health line.
 */
static volatile bool console_released;

void interactive_console_release(void) {
    console_released = true;
}

bool interactive_console_is_released(void) {
...
static const aegis_console_runtime_asset_t runtime_assets[] = {
    {"aegis-init",        "native-userland", "/sbin/aegis-init",           "real ELF executed at EL0 as PID 1"},
    ...
    {"service-manager",   "native-userland", "/sbin/service-manager",      "native EL0 bootstrap process; supervises aegisd"},
    {"aegisd",            "native-userland", "/sbin/aegisd",               "native EL0 daemon bootstrap; IPC health endpoint active"},
...
static void cmd_help(void) {
    ...
    con_puts("  storage              block devices, partitions, persistent filesystem\n");
    con_puts("  sockets              native socket/listener state\n");
    con_puts("  install-nvme CONFIRM copy the installer block device to NVMe with verify\n");
    con_puts("  poweroff             orderly service stop, storage flush, PSCI poweroff\n");
    con_puts("  reboot               orderly service stop, storage flush, PSCI reset\n");
...
static void cmd_status(void) {
    ...
    con_puts("AegisOS v2.0 v58 native-lifecycle runtime\n");
    ...
    con_puts("userland features: "); con_u32_dec(handoff ? handoff->feature_count : 0); con_puts(" ready="); con_u32_dec(handoff ? handoff->ready_count : 0); con_puts(" descriptors="); con_u32_dec(handoff ? handoff->process_descriptor_count : 0); con_puts("\n");
    process_table_stats_t proc = process_table_stats();
    con_puts("native processes: used="); con_u32_dec(proc.used_slots); con_puts(" running="); con_u32_dec(proc.running); con_puts(" zombie="); con_u32_dec(proc.zombie); con_puts(" pid1="); con_puts(proc.pid1_present ? "present" : "missing"); con_puts("\n");
    con_puts("native init image: "); con_puts(elf_loader_state_name()); con_puts(" entry="); con_u32_hex((u32)elf_loader_runtime_entry()); con_puts("\n");
    con_puts("supervisor services: "); con_u32_dec(sup ? sup->service_count : 0); con_puts(" running="); con_u32_dec(sup ? sup->running_count : 0); con_puts(" faulted="); con_u32_dec(sup ? sup->faulted_count : 0); con_puts(" restarts="); con_u32_dec(sup ? sup->total_restarts : 0); con_puts(" total-faults="); con_u32_dec(sup ? sup->total_faults : 0); con_puts("\n");
    ...
    const aegis_block_registry_t *blk = block_storage_state();
    con_puts("storage: devices="); con_u32_dec(blk ? blk->device_count : 0); con_puts(" partitions="); con_u32_dec(blk ? blk->partition_count : 0); con_puts(" backend="); con_puts(block_storage_io_backend_ready() ? "ready" : "missing"); con_puts(" persist="); con_puts(aegisfs_is_writable() ? "rw" : "unavailable"); con_puts("\n");
    con_puts("runtime security: rtc="); con_puts(rtc_ready() ? "ready" : "missing"); con_puts(" entropy="); con_puts(entropy_strong_ready() ? "virtio-strong" : "fallback-only"); con_puts(" listeners="); con_u32_dec(net_socket_listening_count()); con_puts(" connected="); con_u32_dec(net_socket_connected_count()); con_puts("\n");
}
...
static void cmd_services(void) {
    ...
    for (u32 i = 1; i <= max; i++) {
        ...
        if (!svc) continue;
        con_puts("  pid="); con_u32_dec(svc->pid); con_puts(" "); con_puts(svc->name); con_puts(" state="); con_puts(service_supervisor_state_name(svc->state)); con_puts(" faults="); con_u32_dec(svc->fault_count); con_puts(" restarts="); con_u32_dec(svc->restart_count); con_puts("\n");
    }
    con_puts("\nuserland features:\n");
    for (u32 i = 1; i <= userland_feature_count(); i++) {
        const aegis_userland_feature_t *f = userland_get_feature(i);
        if (!f) continue;
        con_puts("  #"); con_u32_dec(f->id); con_puts(" "); con_puts(f->name); con_puts(" -> "); con_puts(f->image_path); con_puts(" state="); con_puts(userland_state_name(f->state));
        if (f->live_pid) { con_puts(" pid="); con_u32_dec(f->live_pid); }
        con_puts("\n");
    }
}
...
static void cmd_rust(void) {
    ...
    con_puts("\nnote: /sbin/aegis-init, /sbin/service-manager, and /sbin/aegisd now execute as distinct native EL0 bootstrap processes. Full Rust service binaries still require the AegisOS Rust target and std/userspace ABI port.\n");
}
...
static void cmd_storage(void) {
    ...
    if (!st) { con_puts("block registry unavailable\n"); return; }
    con_puts("block devices:\n");
    for (u32 i = 0; i < st->device_count; i++) {
        const aegis_block_device_t *d = block_storage_device(i);
        if (!d) continue;
        con_puts("  id="); con_u32_dec(d->id); con_puts(" "); con_puts(d->name);
        con_puts(" state="); con_puts(block_storage_device_state_name(d->state));
        con_puts(" sectors="); con_u32_dec((u32)d->block_count);
        con_puts(" flags="); con_u32_hex(d->flags); con_puts("\n");
    }
    con_puts("partitions:\n");
    for (u32 i = 0; i < st->partition_count; i++) {
        const aegis_block_partition_t *p = block_storage_partition(i);
        if (!p) continue;
        con_puts("  id="); con_u32_dec(p->id); con_puts(" dev="); con_u32_dec(p->device_id);
        con_puts(" "); con_puts(p->name); con_puts(" state="); con_puts(block_storage_partition_state_name(p->state)); con_puts("\n");
    }
    con_puts("AegisFS: mounted="); con_puts(aegisfs_is_mounted()?"yes":"no");
    con_puts(" writable="); con_puts(aegisfs_is_writable()?"yes":"no");
    con_puts(" files="); con_u32_dec(aegisfs_file_count()); con_puts(" generation="); con_u32_dec(aegisfs_generation()); con_puts("\n");
}

static void cmd_sockets(void) {
    con_puts("native sockets: listening="); con_u32_dec(net_socket_listening_count());
    con_puts(" connected="); con_u32_dec(net_socket_connected_count()); con_puts("\n");
}

static void cmd_install_nvme(const char *arg) {
    const char *confirm = skip_spaces(arg);
    if (!streq(confirm, "CONFIRM")) {
        con_puts("refusing destructive install; type: install-nvme CONFIRM\n");
        return;
    }
    con_puts("installing AegisOS source block device to NVMe with sector verification...\n");
    aegis_install_result_t result;
    int rc = aegis_installer_install_to_nvme(&result);
    con_puts("installer rc="); con_u32_dec((u32)(rc < 0 ? -rc : rc));
    con_puts(" source="); con_u32_dec(result.source_device); con_puts(" target="); con_u32_dec(result.target_device);
    con_puts(" copied="); con_u32_dec((u32)result.sectors_copied); con_puts(" crc-src="); con_u32_hex(result.crc_source); con_puts(" crc-dst="); con_u32_hex(result.crc_target); con_puts("\n");
}
...
void interactive_console_run(void) {
    ...
    while (!interactive_console_is_released()) {
        scheduler_yield();
    }
    ...
    for (;;) {
        ...
        if (streq(cmd, "help")) cmd_help();
        ...
        else if (streq(cmd, "storage")) cmd_storage();
        else if (streq(cmd, "sockets")) cmd_sockets();
        else if (starts_with(cmd, "install-nvme")) cmd_install_nvme(cmd + 12);
            ...
            con_puts("requesting orderly AegisOS shutdown...\n");
            service_supervisor_begin_shutdown();
            system_shutdown_request(0, 0);
            system_shutdown_commit();
        }
        else if (streq(cmd, "reboot")) {
            con_puts("requesting orderly AegisOS reboot...\n");
            service_supervisor_begin_shutdown();
            system_shutdown_request(0, 1);
            system_reboot_commit();
        }
