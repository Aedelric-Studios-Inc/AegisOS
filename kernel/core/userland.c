...
int userland_syscall_expansion_selftest(void) {
    ...
    if (syscall_dispatch(SYS_SERVICE_ID, (u64)(uptr)"routerd", 0, 0, 0, 0, 0) <= 0) return AEGIS_EINVAL;
    s64 spawn_pid = syscall_dispatch(SYS_SPAWN, (u64)(uptr)"/sbin/routerd", 0, 0, 0, 0, 0);
