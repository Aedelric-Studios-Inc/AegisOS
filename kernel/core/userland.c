...
const char *userland_state_name(aegis_userland_state_t state) {
    ...
    case AEGIS_USERLAND_NATIVE_RUNNING:return "native-running";
...
const char *user_process_state_name(aegis_user_process_state_t state) {
    ...
    case AEGIS_USER_PROCESS_NATIVE_SCHEDULED:            return "native-scheduled";
    case AEGIS_USER_PROCESS_NATIVE_RUNNING:              return "native-running";
...
const aegis_userland_feature_t *userland_find_feature_by_path(const char *path) {
    if (!path) return NULL;
    for (u32 i = 0; i < handoff.feature_count; i++) {
        if (features[i].state != AEGIS_USERLAND_EMPTY && str_eq(features[i].image_path, path)) {
            return &features[i];
        }
    }
    return NULL;
...
static aegis_userland_feature_t *userland_find_feature_mut(const char *name) {
    if (!name) return NULL;
    for (u32 i = 0; i < handoff.feature_count; i++) {
        if (features[i].state != AEGIS_USERLAND_EMPTY && str_eq(features[i].name, name)) {
            return &features[i];
...
int userland_note_native_process_spawn(const char *name,
                                       const char *path,
                                       u32 pid,
                                       u64 entry,
                                       u64 text_base,
                                       ...
                                       u64 stack_base,
                                       u64 stack_size) {
    if (!name || !path || pid == 0 || entry == 0 || text_base == 0 || text_size == 0 ||
        stack_base == 0 || stack_size == 0) return AEGIS_EINVAL;

    aegis_userland_feature_t *feature_mut = userland_find_feature_mut(name);
    if (!feature_mut || !str_eq(feature_mut->image_path, path)) return AEGIS_ENOENT;
    if (feature_mut->live_pid != 0 && feature_mut->live_pid != pid) return AEGIS_EBUSY;
    feature_mut->live_pid = pid;

    for (u32 i = 0; i < handoff.process_descriptor_count; i++) {
        aegis_user_process_descriptor_t *existing = &user_processes[i];
        if (existing->id == pid || str_eq(existing->name, name)) {
            existing->id = pid;
            existing->entry = entry;
            existing->text.base = text_base;
            existing->text.size = text_size;
            existing->text.flags = AEGIS_USER_REGION_READ | AEGIS_USER_REGION_EXEC |
                                   AEGIS_USER_REGION_USER | AEGIS_USER_REGION_MAPPED;
            existing->stack.base = stack_base;
            existing->stack.size = stack_size;
            existing->stack.flags = AEGIS_USER_REGION_READ | AEGIS_USER_REGION_WRITE |
                                    AEGIS_USER_REGION_USER | AEGIS_USER_REGION_MAPPED;
            existing->initial_sp = (stack_base + stack_size) & ~0xFULL;
            existing->state = AEGIS_USER_PROCESS_NATIVE_SCHEDULED;
            return AEGIS_OK;
        }
    }

    if (handoff.process_descriptor_count >= AEGIS_USERLAND_PROCESS_MAX) return AEGIS_ENOMEM;
    const aegis_userland_feature_t *feature = feature_mut;

    aegis_user_process_descriptor_t *p = &user_processes[handoff.process_descriptor_count++];
    zero_mem(p, sizeof(*p));
    p->id = pid;
    p->feature_id = feature->id;
    p->service_id = feature->service_id;
    p->state = AEGIS_USER_PROCESS_NATIVE_SCHEDULED;
    p->entry = entry;
    p->text.base = text_base;
    p->text.size = text_size;
    p->text.flags = AEGIS_USER_REGION_READ | AEGIS_USER_REGION_EXEC |
                    AEGIS_USER_REGION_USER | AEGIS_USER_REGION_MAPPED;
    p->stack.base = stack_base;
    p->stack.size = stack_size;
    p->stack.flags = AEGIS_USER_REGION_READ | AEGIS_USER_REGION_WRITE |
                     AEGIS_USER_REGION_USER | AEGIS_USER_REGION_MAPPED;
    p->initial_sp = (stack_base + stack_size) & ~0xFULL;
    copy_name(p->name, AEGIS_USERLAND_NAME_MAX, name);
    copy_name(p->image_path, AEGIS_USERLAND_PATH_MAX, path);
    return AEGIS_OK;
}

int userland_mark_feature_native_running(const char *name, u32 pid) {
    aegis_userland_feature_t *feature = userland_find_feature_mut(name);
    if (!feature || pid == 0) return AEGIS_ENOENT;
    if (feature->state != AEGIS_USERLAND_HANDOFF_READY &&
        feature->state != AEGIS_USERLAND_NATIVE_RUNNING) return AEGIS_EBUSY;
    feature->state = AEGIS_USERLAND_NATIVE_RUNNING;
    feature->live_pid = pid;
    for (u32 i = 0; i < handoff.process_descriptor_count; i++) {
        if (user_processes[i].id == pid && str_eq(user_processes[i].name, name)) {
            user_processes[i].state = AEGIS_USER_PROCESS_NATIVE_RUNNING;
            break;
        }
    }
    return AEGIS_OK;
...
int userland_mark_feature_native_exited(const char *name, u32 pid, s32 exit_code) {
    (void)exit_code;
    aegis_userland_feature_t *feature = userland_find_feature_mut(name);
    if (!feature || pid == 0) return AEGIS_ENOENT;
    if (feature->live_pid != pid) return AEGIS_EPERM;

    feature->live_pid = 0;
    feature->state = AEGIS_USERLAND_HANDOFF_READY;
    for (u32 i = 0; i < handoff.process_descriptor_count; i++) {
        aegis_user_process_descriptor_t *p = &user_processes[i];
        if (p->id == pid && str_eq(p->name, name)) {
            p->state = AEGIS_USER_PROCESS_FIRST_EXITED;
            break;
        }
    }
    return AEGIS_OK;
...
int userland_connect_builtin_features(void) {
    ...
    rc = register_feature("service-manager", "aegisd", "/sbin/service-manager",
                          AEGIS_USERLAND_FLAG_REQUIRED | AEGIS_USERLAND_FLAG_PROCESS | AEGIS_USERLAND_FLAG_CONTROL_PLANE);
    if (rc != AEGIS_OK) return rc;
    ...
    if (rc != AEGIS_OK) return rc;
    rc = register_feature("dashboard", "aegisd", "/sbin/dashboard",
                          AEGIS_USERLAND_FLAG_PROCESS | AEGIS_USERLAND_FLAG_CONTROL_PLANE);
    if (rc != AEGIS_OK) return rc;
