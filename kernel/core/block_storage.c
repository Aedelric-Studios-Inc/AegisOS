...
int block_storage_register_v40_flash_layout(void) {
    ...
    u32 dev_id = 0;
    int rc = register_device("aegis-flash0", 512, 131072,
    ...
    if (rc != AEGIS_OK) return rc;
    rc = register_partition(dev_id, "AEGIS_ROOT", 34816, 65536,
                            AEGIS_BLOCK_FLAG_ROOT | AEGIS_BLOCK_FLAG_WRITABLE);
    if (rc != AEGIS_OK) return rc;
    rc = register_partition(dev_id, "AEGIS_CONFIG", 100352, 16384,
