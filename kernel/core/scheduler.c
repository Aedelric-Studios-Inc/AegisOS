...
void scheduler_yield(void) {
    ...
    task_t *cursor = prev->next ? prev->next : task_list_head;
    task_t *next = NULL;
    task_t *idle_fallback = NULL;

    /* Do not enter the idle WFI task while any ordinary task can run.  A
     * cooperative yield from an EL0 SVC executes with IRQs masked; selecting
     * idle at that point would otherwise sleep forever before PID 2 can
     * consume PID 3's IPC health message.
     */
    while (cursor != prev) {
        if (cursor->state == TASK_READY) {
            if (cursor == idle_task) {
                idle_fallback = cursor;
            } else {
                next = cursor;
                break;
            }
        }
        cursor = cursor->next ? cursor->next : task_list_head;
    }

    if (!next) next = idle_fallback;
    if (!next) {
        if (prev->state == TASK_DEAD) {
            PANIC("dead task yielded with no ready successor");
        }
        return;
    }
