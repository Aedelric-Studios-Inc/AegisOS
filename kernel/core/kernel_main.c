...
static void aegisbox_init_prepare_interactive_runtime_state_or_panic(void) {
    /* v55.1.9: shell-first runtime boot.
     *
     * The v38-v55 functions above are formal smoke-proof/milestone functions.
     * They validate contracts and some of them deliberately exercise syscall,
     * IPC, supervisor, page-fault, and EL0 paths that are not safe to run as a
     * blocking pre-shell boot path.  Running them before ttyAMA0 caused the
     * one-stage-at-a-time stalls seen on real QEMU release boots.
     *
     * Normal release boots now bring up VFS, initramfs/rootfs bridge, kernel
     * services, and the interactive shell first.  The full milestone ladder is
     * compiled and executed only in AEGISOS_QEMU_SMOKE proof builds, where it
     * belongs and where the harness can trace it safely.
     */
    printk("[AegisOS:init] interactive runtime: shell-first boot; proof chain deferred to smoke build\n");
}
#endif
...
#endif
#ifndef AEGISOS_QEMU_SMOKE
    aegisbox_init_prepare_interactive_runtime_state_or_panic();
