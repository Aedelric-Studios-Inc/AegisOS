...
#define AEGISOS_INIT_PROOF_LOG(...) printk(__VA_ARGS__)
#else
#define AEGISOS_INIT_PROOF_LOG(...) do { if (0) printk(__VA_ARGS__); } while (0)
#endif
...
static void kernel_idle_task(void) {
    for (;;) {
        /* A task switch originating in an EL0 SVC inherits masked IRQ state.
         * Never execute WFI in that state, or the cooperative scheduler cannot
         * reach another runnable native process.  The scheduler also avoids
         * selecting idle while a non-idle task is ready.
         */
        if (irq_global_is_enabled()) {
...
static void aegisbox_init_mount_vfs_or_panic(void) {
    ...
    printk("[AegisOS:initramfs] installing release rootfs bridge\n");
    ...
    if (block_storage_io_backend_ready()) {
        rc = aegisfs_mount_persistent();
        printk("[AegisOS:storage] AegisFS mount rc=%d writable=%u files=%u generation=%u\n",
               rc,
               aegisfs_is_writable() ? 1U : 0U,
               (unsigned int)aegisfs_file_count(),
               (unsigned int)aegisfs_generation());
        if (rc != AEGIS_OK || !aegisfs_is_mounted() || !aegisfs_is_writable()) {
            PANIC("persistent AegisFS mount failed");
        }
        rc = aegisfs_selftest();
        if (rc != AEGIS_OK) PANIC("persistent AegisFS read/write selftest failed");
        printk("[AegisOS:storage] AegisFS persistence proof passed\n");
    } else {
        printk("[AegisOS:storage] warning: no writable block backend; /persist unavailable\n");
    }
...
static void aegisbox_init_prepare_userland_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] connect features rc=%d count=%u bound=%u\n",
           rc,
...
#ifdef AEGISOS_QEMU_SMOKE
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] handoff prepare rc=%d features=%u ready=%u syscalls=%u mounted=%u\n",
           rc,
     ...
     */
    (void)handoff;
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] handoff prepare complete\n");
#endif
    if (rc != AEGIS_OK) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] selftest rc=%d init=%s router=%s admin=%s\n",
           rc,
...
static void aegisbox_init_prepare_pre_el0_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] pre-EL0 prepare rc=%d descriptors=%u ready=%u\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] first user descriptor name=%s image=%s entry=%p sp=%p state=%s\n",
           first->name,
           first->image_path,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] pre-EL0 selftest rc=%d text=%p stack=%p guard=%p gate=%p\n",
           rc,
...
static void aegisbox_init_load_first_elf_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:elf] load VFS %s rc=%d state=%s entry=%p text=%p size=%p\n",
           image.path,
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:elf] PT_LOAD copy selftest rc=%d copied=%u perms=%u file_bytes=%p zero=%p\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:elf] first process ELF bind rc=%d loader_ready=%u descriptor_state=%s ptload=%u perms=%u\n",
           rc,
...
static void aegisbox_init_prepare_v42_multiprocess_syscalls_or_panic(void) {
    ...
    const aegis_userland_handoff_t *handoff = userland_handoff_state();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] v42 multi-process launch path rc=%d descriptors=%u launchable=%u scheduled=%u\n",
           rc,
           (unsigned int)handoff->process_descriptor_count,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:syscall] v42 expansion prepare rc=%d nr=%u\n",
           rc,
    ...
    handoff = userland_handoff_state();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:syscall] v42 expansion selftest rc=%d spawn_requests=%u last_spawn_pid=%u\n",
           rc,
...
static void aegisbox_init_prepare_v43_ipc_service_messaging_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:ipc] v43 service messaging rc=%d mailboxes=%u channels=%u routes=%u last_channel=%u\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:ipc] v43 message roundtrip rc=%d channel=%u bytes=%u ready=%u\n",
           rc,
           (unsigned int)ipc->last_channel_id,
...
static void aegisbox_init_prepare_v44_service_supervisor_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:supervisor] v44 register rc=%d services=%u registry=%u\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:supervisor] v44 start rc=%d running=%u/%u\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:supervisor] v44 page fault proof rc=%d faults=%u faulted=%u last_pid=%u far=%p router=%s kind=%s\n",
           rc,
...
#else
    /* In the interactive release VM, keep services running.  The controlled
     * page-fault injection proof is smoke-only because it deliberately marks
     * routerd faulted and can block practical tty bring-up.
     */
    if (!service_supervisor_registry_ready()) {
        PANIC("v44 service supervisor interactive registry missing");
    }
#endif
}
...
static void aegisbox_init_prepare_v45_page_isolation_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:mmu] v45 page isolation rc=%d spaces=%u roots=%u mapped=%u text_rx=%u rw_nx=%u guards=%u denied=%u first_ttbr0=%p\n",
           rc,
...
static void aegisbox_init_prepare_v46_network_control_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:network] v46 bring-up rc=%d profile=%s wan=%s lan=%s link=%s lease=%p gw=%p dns=%p routes=%u fw_rules=%u nat=%u gen=%u\n",
           rc,
           net->profile,
           net->wan_if,
           net->lan_if,
...
static void aegisbox_init_prepare_v47_developer_preview_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:release] v47 Developer Preview rc=%d variants=%u enabled=%u manifest=%u docs=%u generation=%u\n",
           rc,
...
static void aegisbox_init_prepare_v48_release_polish_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:release] v48 polish rc=%d boards=%u router_capable=%u services=%u enabled=%u gates=%u generation=%u\n",
           rc,
...
static void aegisbox_init_prepare_v55_release_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:release] v55 final release rc=%d variants=%u services=%u hardening=%u docs=%u checksums=%u generation=%u\n",
           rc,
...
static void aegisbox_init_prepare_v41_storage_process_or_panic(void) {
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:block] v40 flash layout register rc=%d devices=%u partitions=%u persistent=%u\n",
           rc,
           (unsigned int)bst->device_count,
           (unsigned int)bst->partition_count,
           (unsigned int)bst->persistent_partition_count);
    AEGISOS_INIT_PROOF_LOG("[AegisOS:block] partitions boot=%s root=%s config=%s\n",
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:process] v41 table cleanup rc=%d slots=%u used=%u running=%u zombie=%u next_pid=%u\n",
           rc,
...
static void aegisbox_init_prepare_v38_user_processes_or_panic(void) {
    ...
    const aegis_userland_handoff_t *handoff = userland_handoff_state();
    const aegis_user_process_descriptor_t *first = userland_first_process_descriptor();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] argv/envp stack bootstrap rc=%d argc=%u envc=%u sp=%p state=%s\n",
           rc,
    ...
    handoff = userland_handoff_state();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] multiple process descriptors rc=%d total=%u secondary=%u launchable=%u\n",
           rc,
           (unsigned int)handoff->process_descriptor_count,
...
__attribute__((unused)) static void aegisbox_init_controlled_el0_transition_or_panic(void) {
    ...
    int rc = userland_prepare_controlled_el0_transition(
        ...
        first->initial_sp);
    const aegis_userland_handoff_t *handoff = userland_handoff_state();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] controlled EL0 transition prepare rc=%d entry=%p sp=%p active=%u\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] controlled EL0 preflight rc=%d descriptor_state=%s\n",
           rc,
           user_process_state_name(userland_first_process_descriptor()->state));
    if (rc != AEGIS_OK) {
...
static void aegisbox_init_first_tiny_user_process_or_panic(void) {
    ...
    int rc = userland_prepare_first_tiny_user_process(
        ...
        first->initial_sp);
    const aegis_userland_handoff_t *handoff = userland_handoff_state();
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] first tiny user process prepare rc=%d pid=%u entry=%p sp=%p state=%s\n",
           rc,
    ...
    AEGISOS_INIT_PROOF_LOG("[AegisOS:userland] first tiny user process selftest rc=%d launch_ready=%u\n",
           rc,
...
#endif /* AEGISOS_QEMU_SMOKE */

#ifndef AEGISOS_QEMU_SMOKE
static void aegisbox_native_console_task(void) {
    interactive_console_run();
}

static void aegisbox_init_launch_native_pid1_or_panic(void) {
    /* v57 native supervisor-chain bootstrap.
     *
     * This path intentionally runs inside the AegisOS guest.  It does not use
     * Linux systemd, host sockets, NetworkManager, nftables, or the laptop's
     * process model.  The first real ELF is loaded from the AegisOS initramfs,
     * bound to PID 1, entered at EL0, and serviced exclusively through the
     * AegisOS syscall/exception path.
     */
    printk("[AegisOS:native] stage=handoff-init\n");
    userland_handoff_init();

    printk("[AegisOS:native] stage=feature-connect\n");
    int rc = userland_connect_builtin_features();
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] feature-connect rc=%d\n", rc);
        PANIC("native userland feature connection failed");
    }

    printk("[AegisOS:native] stage=handoff-prepare\n");
    rc = userland_prepare_handoff();
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] handoff-prepare rc=%d\n", rc);
        PANIC("native userland handoff failed");
    }

    printk("[AegisOS:native] stage=pre-el0-contract\n");
    rc = userland_prepare_pre_el0_contract();
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] pre-el0-contract rc=%d\n", rc);
        PANIC("native pre-EL0 contract failed");
    }

    printk("[AegisOS:native] stage=native-runtime-init\n");
    service_ipc_init();
    service_supervisor_init();

    printk("[AegisOS:native] stage=elf-loader-init\n");
    elf_loader_init();
    aegis_elf_image_info_t image;

    printk("[AegisOS:native] stage=elf-load path=/sbin/aegis-init\n");
    rc = elf_loader_load_vfs_aegis_init(&image);
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] elf-load rc=%d\n", rc);
        PANIC("native /sbin/aegis-init ELF load failed");
    }

    printk("[AegisOS:native] stage=elf-selftest\n");
    if (elf_loader_selftest() != AEGIS_OK ||
        elf_loader_pt_load_selftest() != AEGIS_OK) {
        PANIC("native /sbin/aegis-init ELF selftest failed");
    }

    printk("[AegisOS:native] stage=descriptor-bind\n");
    rc = userland_bind_first_process_to_loaded_elf(image.path,
                                                   image.entry,
                                                   image.text_vaddr,
                                                   image.text_memsz,
                                                   ...
                                                   image.text_zero_bytes);
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] descriptor-bind rc=%d\n", rc);
        PANIC("native PID 1 ELF descriptor bind failed");
    }

    u64 runtime_entry = elf_loader_runtime_entry();
    u64 runtime_stack_base = elf_loader_runtime_stack_base();
    u64 runtime_stack_size = elf_loader_runtime_stack_size();
    u64 runtime_stack_top = elf_loader_runtime_stack_top();
    if (!runtime_entry || !runtime_stack_base || !runtime_stack_size || !runtime_stack_top) {
        PANIC("native PID 1 runtime image missing");
    }

    printk("[AegisOS:native] stage=process-bind\n");
    process_t *pid1 = process_bind_current_user_image("aegis-init",
                                                      runtime_entry,
                                                      ...
                                                      image.runtime_slot,
                                                      0);
    if (!pid1 || process_getpid() != 1U) {
        PANIC("failed to bind native aegis-init as PID 1");
    }
    rc = userland_note_native_process_spawn("aegis-init",
                                            "/sbin/aegis-init",
                                            1U,
                                            runtime_entry,
                                            image.text_kernel_backing,
                                            ...
                                            runtime_stack_size);
    if (rc != AEGIS_OK) {
        printk("[AegisOS:native] pid1-descriptor rc=%d\n", rc);
        PANIC("failed to publish native PID 1 descriptor");
    }

    /* Log before allocating the successor task so a scheduler/list failure can
     * no longer look like an unexplained pre-launch freeze.
     */
    printk("[AegisOS:native] pid=1 bound entry=%p stack=%p\n",
           (void *)(uptr)runtime_entry,
           (void *)(uptr)runtime_stack_top);

    /* Keep a ready console task in the scheduler before entering EL0.  The
     * task itself remains gated and prints nothing until service-manager has
     * completed the aegisd IPC health proof and calls SYS_CONSOLE_READY.
     */
    printk("[AegisOS:native] stage=console-task-create\n");
    if (!task_create("aegis-console", aegisbox_native_console_task, 50)) {
        PANIC("failed to create post-PID1 console task");
    }

    printk("[AegisOS:native] launching /sbin/aegis-init pid=1 entry=%p stack=%p\n",
           (void *)(uptr)runtime_entry,
           (void *)(uptr)runtime_stack_top);
    aegis_el0_transition_enter(runtime_entry, runtime_stack_top);
    PANIC("native aegis-init returned without SYS_EXIT");
}
#endif
...
#endif
    int update_rc = update_init();
    const aegis_update_state_t *update = update_state();
    printk("[AegisOS:update] metadata rc=%d active=%u pending=%u attempts=%u generation=%u version=%s\n",
           update_rc, (unsigned int)update->active_slot, (unsigned int)update->pending,
           (unsigned int)update->boot_attempts, (unsigned int)update->generation,
...
#ifndef AEGISOS_QEMU_SMOKE
    /* PR1 uses the real virtio/physical Ethernet path.  Failure is surfaced
     * in status and logs but does not prevent the recovery console booting. */
    network_control_init();
    int live_net_rc = network_control_bringup_live(5000U);
    const aegis_network_control_state_t *live_net = network_control_state();
    printk("[AegisOS:network] live bring-up rc=%d link=%s attempts=%u lease=%p gateway=%p dns=%p\n",
           live_net_rc,
           network_control_link_state_name(live_net->wan_link_state),
           (unsigned int)live_net->dhcp_attempts,
           (void *)(uptr)live_net->lease_ip,
           (void *)(uptr)live_net->gateway,
           (void *)(uptr)live_net->dns_server);
    if (live_net_rc == AEGIS_OK && network_control_dhcp_bound()) {
        printk("[AegisOS:network] DHCP bound lease=%p gateway=%p dns=%p\n",
               (void *)(uptr)live_net->lease_ip,
               (void *)(uptr)live_net->gateway,
               (void *)(uptr)live_net->dns_server);
        int socket_rc = net_socket_selftest();
        printk("[AegisOS:network] native socket proof rc=%d\n", socket_rc);
        if (socket_rc != AEGIS_OK) PANIC("native socket API selftest failed");
        printk("[AegisOS:network] native socket proof passed\n");
    }
#endif
#ifdef AEGISOS_QEMU_SMOKE
...
#endif
#ifndef AEGISOS_QEMU_SMOKE
    aegisbox_init_launch_native_pid1_or_panic();
#else
...
void kernel_main(u64 dtb_ptr) {
    ...
    printk("[AegisOS:product] board=%s\n", aegis_board_name());
    ...
    int virtio_rc = virtio_mmio_discover();
    block_storage_init();
    int nvme_rc = nvme_init();
    int blk_rc = virtio_blk_init();
    int layout_rc = AEGIS_ENOENT;
    if (nvme_rc == AEGIS_OK || blk_rc == AEGIS_OK) layout_rc = block_storage_register_v40_flash_layout();
    int eth_rc = ethernet_init();
    net_socket_init();
    entropy_init();
    printk("[AegisOS:devices] virtio=%d count=%u nvme=%d block=%d layout=%d ethernet=%d link=%u rtc=%u rng-strong=%u\n",
           virtio_rc,
           (unsigned int)virtio_mmio_device_count(),
           nvme_rc,
           blk_rc,
           layout_rc,
           eth_rc,
           ethernet_link_ready() ? 1U : 0U,
           rtc_ready() ? 1U : 0U,
           entropy_strong_ready() ? 1U : 0U);
    if (virtio_rc != AEGIS_OK) printk("[AegisOS:devices] warning: no virtio-mmio transports discovered\n");
    if (blk_rc != AEGIS_OK) printk("[AegisOS:storage] warning: virtio-blk unavailable rc=%d\n", blk_rc);
    if (eth_rc != AEGIS_OK) printk("[AegisOS:network] warning: virtio-net unavailable rc=%d\n", eth_rc);
#ifdef AEGISOS_QEMU_SMOKE
    ...
    task_t *idle = task_create("idle", kernel_idle_task, 0);
    if (!idle) {
