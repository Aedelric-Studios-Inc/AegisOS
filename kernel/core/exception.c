...
u64 exception_vector_table_addr(void) {
    return (u64)(uptr)_vectors;
}

int exception_vector_selftest(void) {
    u64 vbar = exception_vector_base_read();
    u64 expected = exception_vector_table_addr();

    if ((expected & 0x7ffULL) != 0) return AEGIS_EINVAL;
    if ((vbar & ~0x7ffULL) != (expected & ~0x7ffULL)) return AEGIS_EINVAL;
    return AEGIS_OK;
}


int exception_decode_fault(u64 esr, u64 far, u64 elr, aegis_exception_fault_t *out) {
    if (!out) return AEGIS_EINVAL;

    out->esr = esr;
    out->far = far;
    out->elr = elr;
    out->ec = (esr >> 26) & 0x3fULL;
    out->iss = esr & 0x01ffffffULL;
    out->fsc = out->iss & 0x3fULL;
    out->write = ((out->iss >> 6) & 0x1ULL) != 0;
    out->instruction_abort = (out->ec == 0x20ULL || out->ec == 0x21ULL);
    out->data_abort = (out->ec == 0x24ULL || out->ec == 0x25ULL);
    out->from_lower_el = (out->ec == 0x20ULL || out->ec == 0x24ULL);
    out->from_current_el = (out->ec == 0x21ULL || out->ec == 0x25ULL);

    /* Translation, access-flag, permission, and alignment faults are treated
     * as page-fault-class exceptions for the v44 supervisor proof.
     */
    out->is_page_fault = false;
    if (out->instruction_abort || out->data_abort) {
        if ((out->fsc >= 0x04ULL && out->fsc <= 0x07ULL) ||
            (out->fsc >= 0x09ULL && out->fsc <= 0x0bULL) ||
            (out->fsc >= 0x0dULL && out->fsc <= 0x0fULL) ||
            out->fsc == 0x21ULL) {
            out->is_page_fault = true;
        }
    }
...
bool exception_fault_is_user_page_fault(const aegis_exception_fault_t *fault) {
    return fault && fault->is_page_fault && fault->from_lower_el;
}
...
void syscall_handler(void *regs) {
...
#ifdef AEGISOS_QEMU_SMOKE
    if (userland_first_tiny_process_active()) {
        u64 elr;
        u64 spsr;
        ...
        u64 arg0 = f->x0;
        s64 ret = syscall_dispatch(f->x8, f->x0, f->x1, f->x2, f->x3, f->x4, f->x5);
        f->x0 = (u64)ret;
        userland_mark_first_user_process_syscall_return(f->x8, arg0, ret, elr, spsr);
        /* v33 returns normally through restore_regs + eret.  If this works,
         * the EL0 body reaches aegis_first_user_process_after_syscall and
         * deliberately traps a second time so the kernel can prove continuation.
         */
        return;
    }

    if (userland_first_tiny_process_return_probe_active()) {
        u64 elr;
        u64 spsr;
        __asm__ volatile("mrs %0, elr_el1" : "=r"(elr));
        __asm__ volatile("mrs %0, spsr_el1" : "=r"(spsr));

        /* v34: after v33 successfully returns to EL0, the tiny aegis-init
         * body immediately issues SYS_EXIT.  We deliberately mark the
         * continuation and then mark the process exited without returning to
         * the dead EL0 context.  The full scheduler/process teardown path is a
         * later milestone; this proof is the safe exit accounting boundary.
         */
        userland_mark_first_user_process_continued_after_syscall(f->x8, f->x0, elr, spsr);
        if (f->x8 == SYS_EXIT) {
            userland_mark_first_user_process_exit(f->x8, f->x0, elr, spsr);
    ...
    if (userland_controlled_el0_transition_active()) {
        u64 elr;
        u64 spsr;
        __asm__ volatile("mrs %0, elr_el1" : "=r"(elr));
        __asm__ volatile("mrs %0, spsr_el1" : "=r"(spsr));
        userland_mark_el0_svc_trap(f->x8, f->x0, elr, spsr);
        /* v31.0 deliberately stops here after proving the EL0 -> EL1 SVC
         * path.  v32 will replace this holding pattern with a real user
         * process continuation/exit path.
         */
        for (;;) {
            __asm__ volatile("wfe" ::: "memory");
        }
