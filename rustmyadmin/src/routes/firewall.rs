//! firewall route for rustmyadmin.

use crate::aegisd_client::AegisdClient;

pub fn index() -> String {
    let data = AegisdClient::query("/firewall/rules").unwrap_or_else(|e| {
        format!(r#"{{"error":"{}"}}"#, e)
    });
    format!(
        r#"<!DOCTYPE html><html><head><meta charset="utf-8">
<title>AegisOS Admin - Firewall</title>
...
pub fn api_list() -> String {
    AegisdClient::query("/firewall/rules").unwrap_or_else(|e| {
        format!(r#"{{"error":"{}"}}"#, e)
    })
}
