//! vpn route for rustmyadmin.

use crate::aegisd_client::AegisdClient;

pub fn index() -> String {
    let data = AegisdClient::query("/vpn/status").unwrap_or_else(|e| {
        format!(r#"{{"error":"{}"}}"#, e)
    });
    format!(
        r#"<!DOCTYPE html><html><head><meta charset="utf-8">
<title>AegisOS Admin - VPN</title>
...
pub fn api_list() -> String {
    AegisdClient::query("/vpn/status").unwrap_or_else(|e| {
        format!(r#"{{"error":"{}"}}"#, e)
    })
}
