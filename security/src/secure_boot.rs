...
fn fe_sub(a: &Fe, b: &Fe) -> Fe {
    ...
    let p2 = [0xFFFFFFFFFFFDA_i64*2, 0xFFFFFFFFFFFFE_i64*2,
              0xFFFFFFFFFFFFE_i64*2, 0xFFFFFFFFFFFFE_i64*2,
              0xFFFFFFFFFFFFE_i64*2];
    [a[0]+p2[0]-b[0], a[1]+p2[1]-b[1], a[2]+p2[2]-b[2],
     a[3]+p2[3]-b[3], a[4]+p2[4]-b[4]]
}
...
fn fe_reduce(h: &mut Fe) {
    ...
    for _ in 0..2 {
        let c0 = h[0] >> 51; h[1] += c0; h[0] &= mask51;
        let c1 = h[1] >> 51; h[2] += c1; h[1] &= mask51;
        let c2 = h[2] >> 51; h[3] += c2; h[2] &= mask51;
        let c3 = h[3] >> 51; h[4] += c3; h[3] &= mask51;
        let c4 = h[4] >> 51; h[0] += c4 * 19; h[4] &= mask51;
    }
}
...
fn fe_pow22523(z: &Fe) -> Fe {
    ...
    t1 = fe_sq(&t0);
    for _ in 1..5 { t1 = fe_sq(&t1); }
    t0 = fe_mul(&t1, &t0);         // z^(2^10-1)
    t1 = fe_sq(&t0);
    for _ in 1..10 { t1 = fe_sq(&t1); }
    t1 = fe_mul(&t1, &t0);         // z^(2^20-1)
    let mut t2 = fe_sq(&t1);
    for _ in 1..20 { t2 = fe_sq(&t2); }
    t1 = fe_mul(&t2, &t1);         // z^(2^40-1)
    t1 = fe_sq(&t1);
    for _ in 1..10 { t1 = fe_sq(&t1); }
    t0 = fe_mul(&t1, &t0);         // z^(2^50-1)
    t1 = fe_sq(&t0);
    for _ in 1..50 { t1 = fe_sq(&t1); }
    t1 = fe_mul(&t1, &t0);         // z^(2^100-1)
    t2 = fe_sq(&t1);
    for _ in 1..100 { t2 = fe_sq(&t2); }
...
fn fe_from_bytes(s: &[u8; 32]) -> Fe {
    let load = |b: &[u8]| -> i64 {
        let mut v = 0u64;
        for (i, &x) in b.iter().enumerate() { v |= (x as u64) << (i * 8); }
        v as i64
    };
    let mask51: i64 = (1i64 << 51) - 1;
    [
...
fn fe_is_negative(f: &Fe) -> bool { (fe_to_bytes(f)[0] & 1) != 0 }
...
fn point_double(p: &Point) -> Point { point_add(p, p) }
...
fn scalar_mult(scalar: &[u8; 32], point: &Point) -> Point {
    ...
    for i in (0..256).rev() {
        ...
        let mut qx = q.x; let mut rx = r.x;
        let mut qy = q.y; let mut ry = r.y;
        let mut qz = q.z; let mut rz = r.z;
        let mut qt = q.t; let mut rt = r.t;
        fe_cswap(&mut qx, &mut rx, -bit); fe_cswap(&mut qy, &mut ry, -bit);
        fe_cswap(&mut qz, &mut rz, -bit); fe_cswap(&mut qt, &mut rt, -bit);
        q = Point { x: qx, y: qy, z: qz, t: qt };
        r = Point { x: rx, y: ry, z: rz, t: rt };
        r = point_add(&q, &r);
        q = point_double(&q);
        let mut qx = q.x; let mut rx = r.x;
        let mut qy = q.y; let mut ry = r.y;
...
fn base_point() -> Point {
    ...
    let x   = if fe_is_negative(&r) { fe_sub(&fe_zero(), &r) } else { r };
    let t   = fe_mul(&x, &y);
...
fn decode_point(bytes: &[u8; 32]) -> Option<Point> {
    ...
    if db.iter().any(|&b| b != 0) { return None; }
    let t = fe_mul(&x, &y);
...
fn sha512_of(data: &[u8]) -> [u8; 64] {
    ...
    for &byte in data {
        buf[bf] = byte; bf += 1;
        if bf == 128 { sha512_compress(&mut state, &buf); bf = 0; }
    }
    buf[bf] = 0x80; bf += 1;
    if bf > 112 {
        while bf < 128 { buf[bf] = 0; bf += 1; }
        sha512_compress(&mut state, &buf); bf = 0;
    }
