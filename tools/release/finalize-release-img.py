...
SECTOR = 512
CONFIG_START_LBA = 395264
RELEASE_OVERLAY_OFFSET = CONFIG_START_LBA * SECTOR + 16384
RELEASE_OVERLAY_SIZE = 4096


def write_release_overlay(img: Path, kernel_bin: Path, kernel_elf: Path, router: Path) -> None:
    payload = {
        "schema": "aegisos-v55-release-overlay-v1",
        "release": "AegisOS-v2.0-v55-release-img",
        "runtime": "v55.1-compatible",
        ...
        "boot_model": "kernel-plus-attached-raw-flash-image",
    }
    data = ("AEGISOS_RELEASE_OVERLAY\n" + json.dumps(payload, indent=2, sort_keys=True) + "\n").encode("utf-8")
    if len(data) > RELEASE_OVERLAY_SIZE:
        raise SystemExit(f"error: release overlay too large: {len(data)} bytes")
    with img.open("r+b") as f:
        f.seek(RELEASE_OVERLAY_OFFSET)
        f.write(data)
        f.write(b"\0" * (RELEASE_OVERLAY_SIZE - len(data)))
...
def main() -> int:
    ...
    write_release_overlay(release_img, kernel_bin, kernel_elf, router)
    ...
    manifest = {
        ...
        "source_router_variant": str(router),
        "source_router_variant_sha256": sha256_file(router),
        "release_overlay_offset": RELEASE_OVERLAY_OFFSET,
        "release_overlay_size": RELEASE_OVERLAY_SIZE,
        "promoted_milestones": [
