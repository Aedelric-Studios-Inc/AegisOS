...
VARIANT_OVERLAY_OFFSET = CONFIG_START_LBA * SECTOR + 4096
VARIANT_OVERLAY_SIZE = 4096


def write_variant_overlay(img: Path, key: str, info: dict, kernel_bin_sha: str, kernel_elf_sha: str) -> None:
    payload = {
        "schema": "aegisos-v55-variant-overlay-v1",
        "variant_key": key,
        "variant": info,
        "kernel_bin_sha256": kernel_bin_sha,
        "kernel_elf_sha256": kernel_elf_sha,
        "runtime_services": info.get("services", []),
    }
    data = ("AEGISOS_VARIANT_OVERLAY\n" + json.dumps(payload, indent=2, sort_keys=True) + "\n").encode("utf-8")
    if len(data) > VARIANT_OVERLAY_SIZE:
        raise SystemExit(f"error: variant overlay too large for {key}: {len(data)} bytes")
    with img.open("r+b") as f:
        f.seek(VARIANT_OVERLAY_OFFSET)
        f.write(data)
        f.write(b"\0" * (VARIANT_OVERLAY_SIZE - len(data)))
...
def main() -> int:
    ...
    for key, info in VARIANTS.items():
        ...
        write_variant_overlay(img, key, info, kernel_bin_sha, kernel_elf_sha)
            ...
            json.dumps(
                {
                    ...
                    "variant_overlay_offset": VARIANT_OVERLAY_OFFSET,
                    "variant_overlay_size": VARIANT_OVERLAY_SIZE,
                    "kernel_bin_sha256": kernel_bin_sha,
                    "kernel_elf_sha256": kernel_elf_sha,
