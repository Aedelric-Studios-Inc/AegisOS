...
#include "aegis_init_elf.h"
#include "aegis_service_manager_elf.h"
#include "aegis_aegisd_elf.h"
#ifdef AEGISOS_NATIVE_RUST_FULL
#include "aegis_dashboard_elf.h"
#include "aegis_rustmyadmin_elf.h"
#endif
...
int initramfs_install_builtin_userland(void) {
    ...
    if (rc != AEGIS_OK) return rc;
    rc = initramfs_add_entry("sbin/aegis-init",
                             aegis_native_init_elf,
                             (u64)aegis_native_init_elf_len,
                             0100755,
                             false);
    if (rc != AEGIS_OK) return rc;
    rc = initramfs_add_entry("sbin/service-manager",
                             aegis_native_service_manager_elf,
                             (u64)aegis_native_service_manager_elf_len,
                             0100755,
                             false);
    if (rc != AEGIS_OK) return rc;
    rc = initramfs_add_entry("sbin/aegisd",
                             aegis_native_aegisd_elf,
                             (u64)aegis_native_aegisd_elf_len,
                             0100755,
                             false);
    if (rc != AEGIS_OK) return rc;
#ifdef AEGISOS_NATIVE_RUST_FULL
    rc = initramfs_add_entry("sbin/dashboard", aegis_native_dashboard_elf,
                             (u64)aegis_native_dashboard_elf_len, 0100755, false);
    if (rc != AEGIS_OK) return rc;
    rc = initramfs_add_entry("opt", NULL, 0, 0040755, true);
    if (rc != AEGIS_OK && rc != AEGIS_EBUSY) return rc;
    rc = initramfs_add_entry("opt/aegisos", NULL, 0, 0040755, true);
    if (rc != AEGIS_OK && rc != AEGIS_EBUSY) return rc;
    return initramfs_add_entry("opt/aegisos/rustmyadmin", aegis_native_rustmyadmin_elf,
                               (u64)aegis_native_rustmyadmin_elf_len, 0100755, false);
#else
    return AEGIS_OK;
#endif
}
...
static const char rootfs_aegisos_toml[] =
    "[aegisos]\n"
    "version = \"v2.0-v57-native-supervisor-runtime\"\n"
    "edition = \"release\"\n"
    "console = \"ttyAMA0\"\n"
...
static const char rootfs_rust_bridge_manifest[] =
    "AegisOS v57 native supervisor bridge manifest\n"
    "userland/init -> /sbin/aegis-init\n"
    "userland/shell -> /bin/aegis-shell\n"
    "userland/aegisctl -> /bin/aegisctl\n"
    "userland/netctl -> /bin/netctl\n"
    "userland/service-manager -> /sbin/service-manager [native EL0]\n"
    "system/daemon/aegisd -> /sbin/aegisd [native EL0]\n"
    ...
    "NOTE: v57 runs native bootstrap ELFs for aegis-init, service-manager, and aegisd. The full Rust implementations still require the AegisOS Rust target and std/userspace ABI port.\n";
...
static const char rootfs_shell_stub[] =
    ...
    "Aegis shell is provided by the v57 kernel-hosted serial console bridge while native shell promotion remains pending.\n"
