...
fn nft_baseline(cfg: &RouterConfig) -> String {
    let host_safe = std::env::var("AEGISOS_HOST_SAFE_FIREWALL").as_deref() == Ok("1");
    let mut input_rules = vec![
        "    iifname \"lo\" accept".to_owned(),
        "    ct state established,related accept".to_owned(),
        "    ip protocol icmp accept".to_owned(),
    ];
    let mut forward_rules = vec!["    ct state established,related accept".to_owned()];
    let mut nat_rules = Vec::new();

    if network::interface_present(&cfg.lan) {
        input_rules.push(format!(
            "    iifname \"{}\" tcp dport {{ 22, 4090, 8443 }} accept",
            cfg.lan
        ));
        input_rules.push(format!(
        ...
        if network::interface_present(&cfg.wan) {
            forward_rules.push(format!(
                "    iifname \"{}\" oifname \"{}\" accept",
                cfg.lan, cfg.wan
            ));
        }
        if network::interface_present(&cfg.wwan) {
            forward_rules.push(format!(
                "    iifname \"{}\" oifname \"{}\" accept",
                cfg.lan, cfg.wwan
            ));
    ...
    if network::interface_present(&cfg.wan) {
        nat_rules.push(format!(
            ...
            cfg.lan_subnet, cfg.wan
        ));
    }
    if network::interface_present(&cfg.wwan) {
        nat_rules.push(format!(
            "    ip saddr {} oifname \"{}\" masquerade",
            cfg.lan_subnet, cfg.wwan
        ));
    }
    ...
    if host_safe {
        if network::interface_present(&cfg.lan) {
            forward_rules.push(format!(
                "    iifname \"{}\" counter log prefix \"AEGIS-DROP-FWD \" drop",
                cfg.lan
            ));
            forward_rules.push(format!(
                "    oifname \"{}\" counter log prefix \"AEGIS-DROP-FWD \" drop",
                cfg.lan
            ));
        }
    } else {
        input_rules.push("    counter log prefix \"AEGIS-DROP-IN \" drop".to_owned());
        forward_rules.push("    counter log prefix \"AEGIS-DROP-FWD \" drop".to_owned());
    }

    let input_policy = if host_safe { "accept" } else { "drop" };
    let forward_policy = if host_safe { "accept" } else { "drop" };
    let nat_table = if nat_rules.is_empty() {
        String::new()
    } else {
        format!(
...
fn delete_table(family: &str, table: &str) {
