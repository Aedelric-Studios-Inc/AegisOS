...
pub fn get_status() -> String {
    ...
    let software_ready = modemmanager && networkmanager;
    let hardware_present = !modems.is_empty();
    let state = if !software_ready {
    ...
    format!(
        r#"{{"radio":{{"backend":"{}","network_backend":"{}","modems_detected":{},"primary_modem":"{}","carrier_profile":"{}","apn":"{}","detail":"{}","software_ready":{},"hardware_present":{},"hardware_required":"USB or M.2 LTE modem with SIM","state":"{}","ready":{}}}}}"#,
        ...
        hardware_present,
        state,
        software_ready && hardware_present
    )
}
...
pub fn connect() -> String {
    ...
    if !network::command_available("nmcli") {
        return r#"{"radio_connect":{"ok":false,"state":"software-unavailable","error":"nmcli not installed"}}"#.to_owned();
    }
    if !network::command_available("mmcli") {
        return r#"{"radio_connect":{"ok":false,"state":"software-unavailable","error":"mmcli not installed"}}"#.to_owned();
    }
    if modem_paths().is_empty() {
        return r#"{"radio_connect":{"ok":false,"state":"hardware-missing","error":"no LTE modem detected; attach a compatible USB or M.2 modem with SIM"}}"#.to_owned();
    }
    if network::dry_run() {
