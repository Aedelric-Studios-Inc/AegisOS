...
//!   aegisctl health
//!   aegisctl security baseline|audit|incidents|enforce
...
fn socket_path() -> String {
    env::var("AEGISD_SOCKET").unwrap_or_else(|_| DEFAULT_SOCKET_PATH.to_owned())
}
...
fn main() {
    ...
    match args[0].as_str() {
        ...
        "network" => network::run(&args[1..]),
        "router" => network::router(&args[1..]),
        "radio" => network::radio(&args[1..]),
        "dns" => network::dns(&args[1..]),
        "vpn" => cmd_vpn(&args[1..]),
        ...
        "security" => security::run(&args[1..]),
        _ => {
            eprintln!("aegisctl: unknown command '{}'", args[0]);
            print_usage();
            std::process::exit(1);
        }
    }
...
fn print_usage() {
    ...
    eprintln!("  aegisctl firewall   list|apply|add [options]|del <id>");
    eprintln!("  aegisctl network    interfaces|apply");
    eprintln!("  aegisctl router     status|provision");
    eprintln!("  aegisctl radio      status|connect|disconnect");
    eprintln!("  aegisctl dns        status|reload");
    ...
    eprintln!("  aegisctl security   baseline|audit|incidents|enforce");
}

fn cmd_vpn(args: &[String]) {
    let sub = args.first().map(String::as_str).unwrap_or("status");
    match sub {
        "status" => cmd_simple("/vpn/status", &[]),
        "up" => match daemon_post("/vpn/up", "") { Ok(json) => println!("{}", json), Err(e) => { eprintln!("aegisctl vpn up: {}", e); std::process::exit(1); } },
        "down" => match daemon_post("/vpn/down", "") { Ok(json) => println!("{}", json), Err(e) => { eprintln!("aegisctl vpn down: {}", e); std::process::exit(1); } },
        _ => {
            eprintln!("aegisctl vpn: unknown sub-command '{}'", sub);
            eprintln!("  sub-commands: status, up, down");
            std::process::exit(1);
        }
    }
}
