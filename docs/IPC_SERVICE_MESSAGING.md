...
V42 proved that the early userland service descriptors are launchable and that the syscall table has process/service control calls. V43 proves those services can be connected through mailbox channels and that the message syscall path can move a payload through a kernel queue safely.
...
SYS_SEND_MSG / SYS_RECV_MSG roundtrip proof
...
→ SYS_SEND_MSG payload queued
→ SYS_RECV_MSG payload received
