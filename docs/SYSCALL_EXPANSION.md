...
The implementation is deliberately kernel-safe: calls validate user pointer ranges, copy user buffers through bounded copyin/copyout helpers, avoid RWX mappings, operate on known descriptors, and preserve the current proven EL0 launch/exit path.
